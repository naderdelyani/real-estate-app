...
/** Returns a formatted price string with optional /mo suffix for rentals. */
function formatPrice(price: number, type: 'sale' | 'rent'): string {
  return `$${price.toLocaleString()}${type === 'rent' ? '/mo' : ''}`;
}

/** Displays a compact real-estate listing card suitable for grid layouts. */
    ...
    <Card hoverable className="group flex flex-col animate-fade-in">
      ...
      <Link href={`/properties/${id}`} className="block relative h-52 bg-gray-100 overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex gap-1.5">
          ...
          {status !== 'available' && (
            <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold uppercase bg-red-500 text-white capitalize">
              {status}
            </span>
          )}
        </div>
        ...
        <button
          ...
          className="absolute top-3 right-3 p-1.5 rounded-full bg-white/80 backdrop-blur hover:bg-white transition-colors"
        >
          <Heart
            className={`h-4 w-4 transition-colors ${isFavorited ? 'fill-red-500 stroke-red-500' : 'stroke-gray-600'}`}
          />
        </button>
      </Link>
      ...
      <div className="flex flex-col flex-1 p-4">
        ...
        <div className="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between text-sm text-gray-600">
          <span className="flex items-center gap-1"><Bed      className="h-4 w-4 text-gray-400" />{bedrooms} bed</span>
          <span className="flex items-center gap-1"><Bath     className="h-4 w-4 text-gray-400" />{bathrooms} bath</span>
