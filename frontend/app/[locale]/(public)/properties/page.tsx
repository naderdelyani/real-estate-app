...
type Props = { params: { locale: string }; searchParams: SearchParams };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="sticky top-0 z-30 bg-white border-b border-gray-200 shadow-sm">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-3">
          <form className="flex flex-wrap items-center gap-3">
            ...
            <div className="ms-auto flex rounded-xl border border-gray-300 overflow-hidden">
              {(['list', 'map'] as const).map((v) => (
                <a
                  key={v}
                  href={`?${new URLSearchParams({ ...searchParams, view: v }).toString()}`}
                  className={`px-3 py-2 text-sm ${view === v ? 'bg-primary-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}
                >
                  {t(v)}
                </a>
              ))}
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
        <p className="text-sm text-gray-500 mb-6">
          {t('propertiesFound', { count: result.total })}
        </p>
